...
def load_model_safe(path, retries=10, delay=1):
    for i in range(retries):
        try:
            ...
            print(f"Fehler beim Laden des Modells, versuche erneut... ({i+1}/{retries})")
            time.sleep(delay)
    raise Exception("Modell konnte nach mehrfachen Versuchen nicht geladen werden.")
...
def play_single_game(lock, process_id):
    ...
    while True:
        while not is_game_over():
            prev_field = read_field().reshape((9, 9))
            input_state = prev_field.reshape(1, 9, 9, 1).astype(np.float32)

            q_values = local_model.predict(input_state, verbose=0)[0]

            mask = (prev_field == -1).reshape(81,)
            ...
            new_field = read_field().reshape((9, 9))

            revealed_before = count_revealed_cells(prev_field)
            revealed_after = count_revealed_cells(new_field)
            delta = max(0, revealed_after - revealed_before)

            # Belohnung: -100 bei Game Over, sonst pro neuem Feld
            reward = -100 if done else delta * 2

            new_input_state = new_field.reshape(1, 9, 9, 1)
            future_q = local_model.predict(new_input_state, verbose=0)[0]
            ...
            if done:
                ...
                local_model = load_model_safe(MODEL_PATH)
def main():
